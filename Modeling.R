...
# set [dat <- "Simulated"] for simulated data
# alternatively, set [dat <- "NepalExtended"] for real data

dat <- "Simulated" 
load(paste0(dat, ".Rdata"))

# set [model <- "MSP"] for a max-stable process
# alternatively, set [model <- "IMSP"] for an inverted max-stable process
...
if (dat == "Simulated"){
  ...
  Lambda = c(Inf, 2^seq(5, -1, -1), 0) # descending grid, may change according to data
...
cat(" Parameter estimates with stationary model:", round(est.s0[1],3), 
    ...
    "Pairwise log-likelihood for stationary model:", round(pl.s0))
...
if (!cv) {pl.s <- model.ini$pl.s} else if (cv) {
  pl.s <- model.ini$pl.s.mean}
samplePairs.ns <- model.ini$samplePairs.ns

para <- model.ini$est.ini[1:model.ini$R.ini][model.ini$reg.idx.ini]
ggplot(data.frame(S), aes(x = data.frame(S)[ , 1], 
                          y = data.frame(S)[ , 2], fill = para)) +
  geom_tile() + scale_fill_viridis(name = "Sill") + xlab("S1") +  ylab("S2") +
  ggtitle("Initial Values (Sill)")

para <- model.ini$est.ini[1:model.ini$R.ini+model.ini$R.ini][model.ini$reg.idx.ini]
ggplot(data.frame(S), aes(x = data.frame(S)[ , 1], 
                          y = data.frame(S)[ , 2], fill = para)) +
  geom_tile() + scale_fill_viridis(name = "Range") + xlab("S1") +  ylab("S2") +
...
# (within hyper)
# pen.type = "L1" or "L2"
# BP.type = the way we generate base partition: "cluster" or "grid"
# ncores = number of cores for parallel computation
# seed = seed number for sampling
# cv = "T" for cross-validation-type or "F" for single holdout set
# fold_n = number of data fold (for data fold case)
# pair.frac = fraction of observation pairs
# R.base = number of subregions in base partition
# Lambda = descending grid of lambda choices
# layers = hierarchical structure for acquiring proper starting point
# stp.len = step length for the partition merging search

# (outside hyper)
# sets = Holdout and Validation, and Training Sets
# model.s = A list of two entries: est.s is the vector of parameter estimates 
#           and pl.s is the pairwise log-likelihood for stationary model
# model.ini = A list of seven entries: samplePairs.ns is a matrix of sampled
#             observation pairs, est.s is the vector of initial choices of the 
#             variogram parameters, pl.s is the pairwise log-likelihood, 
#             
# model.base = A list of six entries: 
# model.merge = A list of eight entries:
